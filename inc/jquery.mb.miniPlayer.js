...
//			, m4a                 : null
		...
		getID3 : function (player) {
			...
			var url = (player.opt.mp3 || player.opt.m4a || player.opt.wav);
		...
		buildPlayer: function (options) {
			...
			this.each(function (idx) {
				...
				var title = $master.data('title') ? $master.data('title') : $master.html();
				...
				// enable available extension
				jQuery.each(player.opt.allowedExtensions, function(idx, ext){
					ext = ext.toLowerCase();
					if(url.indexOf(ext) > 0 && typeof url != "undefined")
					{
						player.opt[ext] = url;
					}
				});
				...
				var fileUrl = encodeURI(player.opt.mp3 || player.opt.m4a || player.opt.wav); // @todo refactor this things
				...
				}).attr("title", player.opt.downloadText+": " + fileName);
